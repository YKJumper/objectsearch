...
vector<Point> detect_motion(
    ...
    const vector<KeyPoint>& keypoints1, const Mat& descriptors1,
    const vector<KeyPoint>& keypoints2, const Mat& descriptors2,
    float s, float es, Point detection_area_left_top, const Ptr<DescriptorMatcher>& matcher, int m
) {
...
int playAndDetect(const string& videoFile, float start_time, float end_time, int fpsStep,
    float crop_percentage, float es, float s, int selectionSide, int m) {
    ...
    while (cap.isOpened() && current_time <= end_time) {
        ...
        if (!curr_descriptors.empty()) {
            objects_coords = detect_motion(prev_gray, curr_gray, prev_keypoints, prev_descriptors, curr_keypoints, curr_descriptors, s, es, detection_area_left_top, matcher, m);
        }
...
int main() {
    playAndDetect("/home/ykuharchuk/projects/objectsearch/FullCars.mp4", 38.0f, 388.0f, 3, 75.0f, 0.5f, 0.5f, 30, 1);
    return 0;
}