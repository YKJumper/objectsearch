...
bitBrightSelector = 0.85 # Initial bright selector value
...
def find_template_positions(image, templates, method=cv2.TM_CCOEFF_NORMED):
    """
    Finds the best match locations of given templates in the reference image.
    """
    positions = []
    for template, original_position in templates:
        res = cv2.matchTemplate(image, template[0], method)
...
def align_images(image1, image2, template_size=75):
    ...
    for method in methods:
        print(f"Aligning images with method {method}")
        template_positions = find_template_positions(image2, templates, method)
        template_positions_ver = find_template_positions(image2, templates_ver, method)

        matched_positions = []
        matched_templates = [] 
        for i in range(len(templates)):
            pos_dst = dist(template_positions[i], template_positions_ver[i])
            if pos_dst<=template_size/2:
...
def get_frame_at_time(cap, fps, time_sec, crop_percentage=18):
...
def process_frames(videoFile, startTime, timeStep, timeDelta, frame_queue, endTime=None, sizeThresh=1):
    ...
    while T1 + timeStep <= endTime:
        ...
        diff = cv2.absdiff(gray1, aligned_image)
...
def resize_and_display(image, screen_width=1920, screen_height=1080, title="Detected Difference", delay=0.5):
    ...
    if width > screen_width or height > screen_height:
        ...
        image_toshow = resized_image
    else: 
        image_toshow = image
    
    cv2.imshow(title, image_toshow)
...
detect_changes("Cars.mp4", startTime=40, timeStep=0.30, timeDelta=0.5, endTime=999, displayTime=2.5, sizeThresh=1)
